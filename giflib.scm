  ...
  (and-let* ((color-map* (color-map-pointer color-map)))
    (let ((count ((foreign-lambda* int (((c-pointer (struct "ColorMapObject")) colormap))
      ...
      (if (and (>= index 0) (< index count))
          (let ((red ((foreign-lambda* unsigned-byte (((c-pointer (struct "ColorMapObject")) colormap)
                                                      (int i))
                                       "C_return(colormap->Colors[i].Red);")
                      color-map* index))
                (green ((foreign-lambda* unsigned-byte (((c-pointer (struct "ColorMapObject")) colormap)
                                                        (int i))
                                         "C_return(colormap->Colors[i].Green);")
                        color-map* index))
                (blue ((foreign-lambda* unsigned-byte (((c-pointer (struct "ColorMapObject")) colormap)
                                                       (int i))
